...
python3 git_hilfsprogramm.py md
"""
...
def ausführen_befehl(befehl, ordner):
    try:
        prozess = subprocess.run(
            befehl, shell=True, text=True, capture_output=True, check=True, cwd=ordner)
        print(prozess.stdout)
        return True
    except subprocess.CalledProcessError as e:
        print(f"Fehler bei Ausführung des Befehls: {
              e.cmd}\nFehlerausgabe:\n{e.stderr}\n{e.stdout}")
        entscheidung = input(
...
def lokales_repo_erstellen_und_initialisieren(ordner):
    ...
    if not os.path.exists(gitignore_pfad):
        with open(gitignore_pfad, 'w') as gitignore:
            gitignore.write(
                "# Hier Dateien und Verzeichnisse angeben, die von der Versionskontrolle ausgeschlossen werden sollen\n")
    ...
    prozess = subprocess.run(
        f"git -C {ordner_pfad} status -s", shell=True, text=True, capture_output=True)
...
def lokales_repo_mit_github_verknüpfen(ordner):
    ...
    prozess = subprocess.run(
        "git remote show origin", shell=True, text=True, capture_output=True, cwd=ordner)
    if "fatal: Remote origin already exists." in prozess.stderr:
        print("Fehler: Externes Repository origin existiert bereits.")
        return

    entscheidung = input(
        "Möchten Sie ein bereits vorhandenes GitHub-Repository verwenden oder ein neues erstellen und verknüpfen? (vorhanden/neu): ").lower()
    ...
    if entscheidung == "vorhanden":
        ...
        subprocess.run(f"git remote add origin {
                       github_repo_url}", shell=True, cwd=ordner)
        subprocess.run("git branch -M main", shell=True, cwd=ordner)
        subprocess.run("git push -u origin main", shell=True, cwd=ordner)
        print(f"Das lokale Repository '{
              ordner}' wurde erfolgreich mit dem GitHub-Repository verknüpft.")
        ...
        print(f"Das lokale Repository '{
              ordner}' wurde erfolgreich erstellt und mit einem neuen GitHub-Repository verknüpft.")
    else:
...
def commiten_und_pushen(ordner):
    ...
    befehl = f"cd {ordner} && git add {
        dateien} && git commit -m '{commit_nachricht}' && git push origin main"
...
def änderungen_commiten(ordner):
    ...
    status_prozess = subprocess.run(
        f"git -C {ordner} status -s", shell=True, text=True, capture_output=True)
...
def repo_klonen(ordner):
    ...
    if repositories:
        ...
        try:
            auswahl = int(input(
                "Geben Sie die Nummer des Repositories ein, das Sie klonen möchten, oder 0 zum Abbrechen: "))

            if 0 < auswahl <= len(repositories):
                repo_name = repositories[auswahl - 1]["name"]

                repo_url = GITHUB_URL + repo_name
                print(f"Klone das Repository '{repo_name}' von der URL '{
                      repo_url}' in den Ordner '{ordner}'.")

                ausführen_befehl(f"git clone '{repo_url}' .", ordner)

                print(f"Repository '{repo_name}' wurde erfolgreich in den Ordner '{
                      ordner}' geklont.")
            elif auswahl == 0:
                print("Befehl wurde abgebrochen.")
            else:
...
def änderungen_pullen(ordner):
    prozess = subprocess.run("git status --porcelain", shell=True, text=True,
    ...
    ausführen_befehl(f"git pull origin {
                     remote_branch}", os.path.join(os.getcwd(), ordner))
...
def branch_wechseln(ordner):
    ...
    ausführen_befehl(f"git checkout {branch_name}",
...
def get_user_repositories(username):
    ...
    if response.status_code == 200:
        ...
        print(f"Fehler beim Abrufen der Repositories für Benutzer {
              username}. Statuscode: {response.status_code}")
        return []
...
def github_repository_löschen():
    ...
    while erneut_ausführen:
        ...
        try:
            ...
            print(f"Repository '{
                  repository_name}' wurde erfolgreich gelöscht.")
        except subprocess.CalledProcessError as e:
            print(f"Fehler bei Ausführung des Befehls: {
                  e.cmd}\nFehlerausgabe:\n{e.stderr}\n{e.stdout}")
            entscheidung = input(

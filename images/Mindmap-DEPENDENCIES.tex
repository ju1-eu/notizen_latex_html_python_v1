...
% Hauptknoten
\node[text width=4cm, font=\large\bfseries]{Python-Projektumgebung}
  child [concept color=MindMapGreen] { node {Python}
    child { node [text only] {Programmiersprache} }
  }
  child [concept color=MindMapRed] { node {Virtuelle-Umgebungen}
    child { node [text only] {Isolation der Abhängigkeiten} }
    child { node [text only] {venv} }
  }
  child [concept color=MindMapOrange] { node {Abhängigkeiten}
    child { node [text only]  {requirements.txt} }
    child { node {Paketverwaltung}
      child { node [text only] {pip} }
      child { node [text only] {Installieren, Aktualisieren, Entfernen} }
    }
  }
  child [concept color=MindMapBlue] { node {Hilfsbibliotheken}
    child { node [text only] {Jinja2} }
    child { node [text only] {Pygments} }
    child { node [text only] {python-dotenv} }
  }
  child [concept color=MindMapPurple] { node {Netzwerk-bibliotheken}
    child { node [text only] {requests} }
    child { node [text only] {urllib3} }
  };
\end{tikzpicture}
\end{document}

...
def check_dependencies():
    ...
    if missing_dependencies:
        print(
            f"Fehlende Abhängigkeiten: {', '.join(missing_dependencies)}. Bitte installieren Sie diese, bevor Sie fortfahren."
        )
        sys.exit(1)
...
def install_extensions(extensions_dict):
    ...
    for extension, description in extensions_dict.items():
        try:
            ...
            print(
                f"Fehler bei der Installation der Erweiterung {extension}. Bitte manuell überprüfen."
            )
...
def create_clang_format_file(project_root):
    ...
    try:
        with open(clang_format_path, "w") as file:
            subprocess.run(
                ["clang-format", "-style=google", "-dump-config"], check=True, stdout=file
            )
